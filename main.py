...
from pokemonutils import (
    ...
    get_nature_increase_decrease,
    get_pokemon_base_stats,
    get_pokemon_max_xp,
)
from savedatautils import get_evs_chunk, get_growth_chunk, get_misc_chunk
from utils import get_offseted, h2i, i2h, set_offseted
...
for pokemon_index in range(pokemon_count):
    ...
    base_hp, base_atk, base_def, base_spe, base_spa, base_spd = get_pokemon_base_stats(
        h2i(species)
    )
    ...
    new_spd = calculate_other_stat(
        "special-defense", base_spe, spd_iv, spd_ev, new_level, nature_name
    )
    ...
    encrypted_xp = h2i(xp) ^ decrypt_key
...
mod_section_contents = set_offseted(mod_section_contents, 0xFF6, new_section_checksum)
